...
def generate_launch_description():
    ...
    # ADD THIS: Distance monitoring node
    ...
    # UPDATED: Camera with depth (when use_depth_input=true)
    camera_feed_depth_node = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(cam_feed_launch_file),
        launch_arguments={
            # OPTIMAL SETTINGS FOR VMWARE WITH DEPTH
            "rgb_camera.color_profile": "1280x720x15",
            "depth_module.depth_profile": "424x240x15",  # Lower depth res to save bandwidth
            "pointcloud.enable": "false",  # Disable to save resources
            "enable_rgbd": "true",
            "enable_sync": "true",
            "align_depth.enable": "true",
            "enable_color": "true",
            "enable_depth": "true",
            "enable_gyro": "false",       # Disable IMU
            "enable_accel": "false",      # Disable accelerometer
            "enable_infra1": "false",     # Disable infrared
            "enable_infra2": "false",     # Disable infrared
            "initial_reset": "true",      # Helps with VMware stability
            "reconnect_timeout": "15.0",  # Longer timeout for VMware
        }.items(),
        condition=IfCondition(LaunchConfiguration('use_depth_input'))
    )

    # UPDATED: Camera without depth (optimal for ArUco detection)
    camera_feed_node = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(cam_feed_launch_file),
        launch_arguments={
            # YOUR OPTIMAL SETTINGS FOR VMWARE
            "rgb_camera.color_profile": "1280x720x15",  # HD resolution, 15 FPS
            "enable_depth": "false",      # No depth stream
            "pointcloud.enable": "false", # No pointcloud
            "enable_color": "true",       # Color stream only
            "enable_gyro": "false",       # Disable IMU
            "enable_accel": "false",      # Disable accelerometer
            "enable_infra1": "false",     # Disable infrared
            "enable_infra2": "false",     # Disable infrared
    ...
    return LaunchDescription([
        ...
        # Nodes
        aruco_node,
        distance_monitor_node,  # ADD THIS LINE
