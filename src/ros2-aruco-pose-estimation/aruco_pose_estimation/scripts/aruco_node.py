...
from tf2_ros import TransformStamped # line 57
...
class ArucoNode(rclpy.node.Node):
    def __init__(self):
        ...
        self.tf_pub = self.create_publisher(TransformStamped, "/tf_aruco", 10) # line : 117
    ...
    def image_callback(self, img_msg: Image):
        ...
        if len(markers.marker_ids) > 0:
            ...
            for i in range(len(markers.marker_ids)): # line : 193
                tf = TransformStamped()
                tf.header.stamp = img_msg.header.stamp
                tf.header.frame_id = "camera_link"
                tf.child_frame_id = f"aruco_{markers.marker_ids[i]}"
                tf.transform.translation.x = pose_array.poses[i].position.x
                tf.transform.translation.y = pose_array.poses[i].position.y
                tf.transform.translation.z = pose_array.poses[i].position.z
                tf.transform.rotation = pose_array.poses[i].orientation
                odometry = tf.transform.translation
